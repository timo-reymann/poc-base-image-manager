...
def test_resolve_default_template(tmp_path):
    ...
    assert path == templates_dir / "Dockerfile.tpl"
...
def test_resolve_explicit_template(tmp_path):
    ...
    templates_dir = tmp_path / "templates"
    templates_dir.mkdir()
    (templates_dir / "custom.tpl").touch()
    (templates_dir / "Dockerfile.tpl").touch()

    resolver = TemplateResolver()
    path = resolver.resolve(templates_dir, explicit="custom.tpl", variant_name=None)

    assert path == templates_dir / "custom.tpl"
...
def test_resolve_variant_template(tmp_path):
    ...
    assert path == templates_dir / "Dockerfile.browser.tpl"
